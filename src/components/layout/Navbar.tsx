    ...
    <>
            ...
            <div className="hidden nav:flex nav:items-center nav:space-x-2">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative px-4 py-3 mx-1 font-bold transition-all duration-300 group ${isActive(item.href)
                    ? 'text-white'
                ...
                >
                  ...
                  {/* Text */}
                  <span className={`relative z-10 text-sm font-bold transition-all duration-300 ${isActive(item.href)
                    ? 'text-white'
            ...
            <div className="nav:hidden flex items-center">
              ...
              >
                ...
                <div className="w-6 h-6 relative flex flex-col justify-center space-y-1">
                  <span
                    className={`w-6 h-0.5 bg-white rounded-full transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-1.5' : ''
                      }`}
                    style={{
                  ...
                  />
                  <span
                    className={`w-6 h-0.5 bg-white rounded-full transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''
                      }`}
                    style={{
                      filter: 'drop-shadow(0 0 5px #743f77) drop-shadow(0 0 10px #743f77)'
                    }}
      ...
      {isMenuOpen && (
        <>
          ...
          {/* Mobile menu panel */}
          <div className="fixed top-20 right-4 left-4 z-50 nav:hidden">
            <div
              ...
              <div className="relative z-10 p-6 space-y-3">
                {navItems.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setIsMenuOpen(false)}
                    className={`relative block px-6 py-4 rounded-xl text-base font-bold transition-all duration-300 group ${isActive(item.href)
                      ? 'text-white'
                      : 'text-white/90 hover:text-white'
                      }`}
                    style={{
